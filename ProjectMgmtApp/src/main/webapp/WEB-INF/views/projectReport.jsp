...
<form action="report.pdf" target="_blank" method="get">
		...
		<table class="table table-striped" id="proj">
			...
			<tr>
				<td>${requestScope.p.projectID }</td>
				<td>${requestScope.p.projectName }</td>
				<td>${requestScope.p.description }</td>
				<td>${requestScope.p.stage }</td>
				
			</tr>
			
		</table>
		
		<h3>Teams Assigned To project</h3>
		<div class="uldiv" align="center">
		<ul class="pteam">
		<c:forEach items="${requestScope.pTeams }" var="pt">
			<li> <c:out value="${pt.teamName }"></c:out> </li></br>
			</c:forEach>
		</ul>
		</div>
		<input type="hidden" value="${requestScope.p.projectID}" name="project"/>
		
		<button type="submit" class="btn btn-inverse">Generate Pdf Report </button><br/><br/>
		<a href="${pageContext.servletContext.contextPath}/manager/manageProject.htm"> &lt;&lt; Click </a> to go back to View all projects
	</form>	
